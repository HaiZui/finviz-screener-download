...
def load_finviz_table(screener_table_name, target_table_name = None):
    ...
    columns, table = fin.screenerTable(screener_table_name)
...
dag = DAG('dag_load_finviz_screener_tables_2', description='Load Finviz screener tables',
                        schedule_interval='00 04 * * *', # Daily at 4 AM
...
# Define success email operator
email_operator_success = EmailOperator(
        task_id='send_email_success',
        to='parviainen.teemu@gmail.com',
        subject='InvestServer: Finviz screener tables loaded successfully',
        html_content=""" <h3>Success</h3> """,
        dag=dag
)

dummy_operator >> price_operator_valuation >> price_operator_financial >> price_operator_ownership >> price_operator_technical >> email_operator_success
